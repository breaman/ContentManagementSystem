    ...
    ILogger<MediaUploadService> logger) : IMediaUploadService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return Invalid(MediaCodes.FileMissing, "The uploaded file is empty.");
        }
        ...
        if (!prepared.IsSuccess) return CmsResult<MediaUploadResult>.Invalid(prepared.Diagnostics);
        ...
        if (options.RequireAltTextOnUpload &&
        ...
        if (!scan.IsClean)
        {
            ...
            return Invalid(MediaCodes.MalwareDetected, "The file was flagged by the malware scanner and refused.");
        }
        ...
        if (existing is not null)
        {
            ...
            return CmsResult<MediaUploadResult>.Success(
                new MediaUploadResult(ToDetail(existing), Deduplicated: true, prepare.Removals));
        }
        ...
        catch (DbUpdateException exception)
        {
            ...
            return CmsResult<MediaUploadResult>.Success(
                new MediaUploadResult(ToDetail(winner), Deduplicated: true, prepare.Removals));
        }
        ...
        return CmsResult<MediaUploadResult>.Success(
            new MediaUploadResult(ToDetail(item), Deduplicated: false, prepare.Removals));
    }
    ...
    private static CmsResult<MediaUploadResult>? TooLong(MediaUploadRequest request)
    {
        if (request.AltText is { Length: > FieldLengths.ShortDescription })
        {
            return Invalid(MediaCodes.TooLong, "The alternative text is too long.", nameof(request.AltText));
        }

        if (request.Title is { Length: > FieldLengths.EntityName })
        {
            return Invalid(MediaCodes.TooLong, "The title is too long.", nameof(request.Title));
        }

        if (request.Caption is { Length: > FieldLengths.Caption })
        {
            return Invalid(MediaCodes.TooLong, "The caption is too long.", nameof(request.Caption));
        }

        return request.Credit is { Length: > FieldLengths.EntityName }
            ? Invalid(MediaCodes.TooLong, "The credit is too long.", nameof(request.Credit))
            : null;
    }
    ...
    private static MediaDetail ToDetail(MediaItem item) => new(
        item.Id,
        item.FolderId,
        item.FileName,
    ...
    private static CmsResult<MediaUploadResult> Invalid(string code, string message, string? path = null) =>
        CmsResult<MediaUploadResult>.Invalid(code, message, path);
