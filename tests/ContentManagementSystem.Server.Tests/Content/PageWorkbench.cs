...
public sealed class PageWorkbench : IAsyncDisposable
{
    ...
    /// <summary>
    /// A zone definition holding one picked media item (task P5-19).
    /// </summary>
    /// <param name="key">The zone key.</param>
    /// <param name="configurationJson">
    /// The picker settings the publish check reads — <c>allowedTypes</c>, <c>minWidth</c>,
    /// <c>aspectRatio</c>, <c>sizes</c> — or null for an unrestricted slot.
    /// </param>
    public static Zone MediaZone(string key, string? configurationJson = null) =>
        new()
        {
            Key = key,
            Name = key,
            FieldTypeKey = FieldTypeKeys.Media,
            ConfigurationJson = configurationJson,
        };

    /// <summary>
    /// Uploads an image through the real pipeline and returns the stored item.
    /// </summary>
    /// <param name="width">Pixel width to generate.</param>
    /// <param name="height">Pixel height to generate.</param>
    /// <param name="cancellationToken">Token observed while uploading.</param>
    /// <param name="altText">The item's own alternative text, or null to leave it undescribed.</param>
    /// <param name="isDecorative">Whether the item renders <c>alt=""</c>.</param>
    /// <remarks>
    /// Through <c>IMediaUploadService</c> rather than by inserting a row, so the item under test has
    /// been sniffed, hashed, and stored the way a real upload is — which is what makes the stored
    /// original reachable through a signed URL later in the same test.
    /// <para>
    /// The generated bytes carry a unique random colour, because the pipeline deduplicates on the
    /// SHA-256 of the content: two fixtures asking for "an 800×600 image" would otherwise be the
    /// same item, and a test arranging two pictures would silently be arranging one.
    /// </para>
    /// <para>
    /// Asking for an undescribed image relaxes the <em>upload</em> alt-text check for that one call.
    /// That is not a way around the rule — it is the only way to reach the state the publish check
    /// exists for. A library imported from a legacy site is full of pictures nobody described, and
    /// the publish check is the one that cannot be skipped (spec section 13.7).
    /// </para>
    /// </remarks>
    public async Task<MediaDetail> AddImageAsync(
        int width,
        int height,
        CancellationToken cancellationToken,
    ...
    {
        var options = Resolve<MediaUploadOptions>();
        var required = options.RequireAltTextOnUpload;

        options.RequireAltTextOnUpload = required && (altText is not null || isDecorative);

        try
        {
            return await UploadFixtureAsync(width, height, altText, isDecorative, cancellationToken);
        }
        finally
        {
            options.RequireAltTextOnUpload = required;
        }
    }

    /// <summary>
    /// Uploads a small PDF through the real pipeline, for the checks that turn on what a file is.
    /// </summary>
    /// <param name="cancellationToken">Token observed while uploading.</param>
    /// <remarks>
    /// A genuine PDF header rather than arbitrary bytes named <c>.pdf</c>: the pipeline sniffs magic
    /// numbers and would refuse the latter, which would make a test about <c>allowedTypes</c> fail
    /// for a reason that has nothing to do with it. The trailer is included so the file is one a
    /// reader would open, though nothing here decodes it.
    /// </remarks>
    public async Task<MediaDetail> AddDocumentAsync(CancellationToken cancellationToken)
    {
        ...
        var result = await Resolve<IMediaUploadService>().UploadAsync(
            new MediaUploadRequest(content, $"fixture-{Guid.NewGuid():N}.pdf"),
            cancellationToken);
    ...
    private async Task<MediaDetail> UploadFixtureAsync(
        int width,
        int height,
        string? altText,
        bool isDecorative,
        CancellationToken cancellationToken)
    {
        await using var content = new MemoryStream(TestImages.Encode(width, height, unique: true));

        var result = await Resolve<IMediaUploadService>().UploadAsync(
            new MediaUploadRequest(
                content,
                $"fixture-{Guid.NewGuid():N}.jpg",
                FolderId: null,
                AltText: altText,
    ...
    /// <summary>
    /// Puts a picked media item into a page's draft zone.
    /// </summary>
    /// <param name="page">The page as it currently stands, for its row version.</param>
    /// <param name="zoneKey">The zone to fill.</param>
    /// <param name="mediaId">The item to place.</param>
    /// <param name="cancellationToken">Token observed while saving.</param>
    /// <param name="altOverride">Alternative text for this placement only, or null to use the item's.</param>
    /// <param name="crop">The placement's crop as a JSON object, or null for the whole picture.</param>
    /// <returns>The page reloaded.</returns>
    public async Task<PageDetail> PlaceMediaAsync(
        PageDetail page,
        string zoneKey,
        int mediaId,
        CancellationToken cancellationToken,
            ...
            { "schemaVersion": 1, "templateKey": "{{page.Summary.TemplateKey}}",
              ...
              "zones": { "{{zoneKey}}":
                { "type": "media", "mediaId": {{mediaId}},
                  "altOverride": {{(altOverride is null ? "null" : JsonSerializer.Serialize(altOverride))}},
                  "crop": {{crop ?? "null"}} } } }
            """;
